...
model = Sequential()
model.add(Dense(units=128, input_shape=(input_length,)))
...
plt.legend()
# display and save
plt.savefig("./training_down.jpg", dpi=2400)
plt.show()

# save the trained model for python
model.save("deeplearing_model_down")
# save the trained model for javascript
import tensorflowjs as tfjs
tfjs.converters.save_keras_model(model, "./mostgraph_jsmodel_down")

# test the model
# labels_test = mostgraph_normal_test_data_numpy[ : , 0:1]
# data_test = mostgraph_normal_test_data_numpy[ : , 1:12]
...
#data_test = mostgraph_abnormal_test_data_numpy[ : , 1:12]
...
plt.legend()
plt.savefig('./roc_down.jpg', dpi=2400)
plt.show()
