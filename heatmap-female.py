...
def judge_abnormal(TargetFileName, DiseaseLabel):
    ...
    plt.savefig("femaleHeatmap"+DiseaseLabel+".jpg", dpi=1200)
    plt.show()
    ...
    plt.savefig("femaleClusterHeatmap"+DiseaseLabel+".jpg", dpi=2400)
    plt.show()
...
JudgedValues_pandas = judge_abnormal("female_control.csv", "Female Normal")
# Convert to numpy
...
# ASTHMA
# return as 0(normal) or 1(disease)
JudgedValues_pandas = judge_abnormal("female_ba.csv", "Female Asthma")
# Convert to numpy
JudgedValues_numpy = JudgedValues_pandas.to_numpy()
# Count the number of abnormal items
